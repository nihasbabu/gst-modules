...
def add_total_row(ws, columns, start_row, end_row):
    ...
    numeric_cols = ['Invoice value', 'Taxable Value', 'Integrated Tax', 'Central Tax',
                    'State/UT Tax', 'Cess', 'Addl. Cost', 'Round Off', 'PURCHASE GST',
                    'Consumables', 'TCS ON SALE  .10%', 'IN 206C(1H) GOODS']
...
def process_purchase_data(input_files, template_file=None, existing_wb=None):
    ...
    all_data = []
    logging.debug("Initialized all_data list")
    ...
    for filepath, branch_key in input_files:
        ...
        if len(wb.sheetnames) > 1:
            ...
            if "Purchase Register" in wb.sheetnames:
                ...
                logging.debug("Selected 'Purchase Register' sheet")
            else:
        ...
        for row in ws.iter_rows(min_row=header_row + 1):
            ...
            supplier_inv_num = row_data.get('Supplier Invoice number')

            if not inv_type or supplier_inv_num is None:
                logging.debug(f"Skipping row: inv_type={inv_type}, supplier_inv_num={supplier_inv_num}")
                continue
    ...
    logging.debug(f"Total records in all_data: {len(all_data)}")
    ...
    logging.debug("Sorting all_data by Invoice date")
    all_data.sort(key=lambda x: x['Invoice date'] if x['Invoice date'] is not None else datetime.datetime.min)
    logging.debug("Sorting all_data completed")

    def sort_key(row):
        supplier = row.get('Supplier Name', '')
        date = row.get('Invoice date', datetime.datetime.min)
        logging.debug(f"Sorting row with supplier: {supplier}, date: {date}")
        if supplier.lower() in ['cash', '(cancelled )'] or not supplier:
            logging.debug(f"Supplier marked as secondary: {supplier}")
            return (1, supplier or '', date)
        logging.debug(f"Supplier marked as primary: {supplier}")
        return (0, supplier, date)

    logging.debug("Sorting all_data for PUR-Total_sws")
    ...
    for sheet_name, data in sheets_to_create:
        ...
        for row_idx, row_data in enumerate(data, start=start_row):
            row = [row_data.get(header, '') if header not in ['Invoice date', 'Supplier Invoice date'] else
    ...
    logging.debug("Processing summary sheet: PUR-Summary-Total")
    ws = create_or_replace_sheet(output_wb, "PUR-Summary-Total",
                                 "Purchase Register - Total - Summary", summary_headers)
    summary_data = {}
    invoice_numbers = set()
    ...
    for row in all_data:
        ...
        logging.debug(f"Processing summary row: inv_num={inv_num}, date={date}")
        if inv_num is None:
            logging.debug("Skipping row: no supplier invoice number")
            continue
        if isinstance(date, datetime.datetime):
            ...
            if inv_num not in invoice_numbers:
                ...
                logging.debug(f"Incremented count for {month}, unique invoice: {inv_num}")
    ...
    if summary_data:
        logging.debug("Adding total row for PUR-Summary-Total")
        total_row = ['Total']
    ...
    logging.info("Created summary sheet PUR-Summary-Total")
